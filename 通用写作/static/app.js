...
function installLocalEnv(){var btn=document.getElementById("installLocalBtn");var prog=document.getElementById("installProgress");btn.disabled=true;btn.innerHTML='<span class="spinner"></span>正在下载...';prog.style.display="block";prog.innerHTML='正在下载 llama-server (约50MB，仅首次需要)...';fetch("/api/install-llama").then(r=>r.json()).then(d=>{if(d.ok){prog.innerHTML='✅ '+d.message;btn.style.display="none";showToast("安装完成！请刷新模型列表");setTimeout(function(){showSettings();},500);}else{prog.innerHTML='❌ '+d.error;btn.disabled=false;btn.innerHTML='📦 一键安装离线运行环境';}}).catch(function(e){prog.innerHTML='❌ 下载失败: '+e.message;btn.disabled=false;btn.innerHTML='📦 一键安装离线运行环境';});}