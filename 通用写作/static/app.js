...
function renderRecommendedModels(models){var groups={};models.forEach(function(m){if(!groups[m.ram])groups[m.ram]=[];groups[m.ram].push(m);});var html='';Object.keys(groups).forEach(function(ram){html+='<div style=\"margin-bottom:12px;\"><div style=\"font-size:11px;color:var(--accent);margin-bottom:4px;\">💡 '+ram+' 内存推荐</div>';groups[ram].forEach(function(m){var dlUrl=m.url.replace('/summary','/resolve/master/')+m.file;html+='<div style=\"padding:8px 10px;margin-bottom:4px;background:var(--bg4);border-radius:6px;font-size:12px;\"><span style=\"color:var(--text-bright);\">'+m.name+'</span> <span style=\"color:var(--text-dim);\">· '+m.brand+' · '+m.size+'</span><br><span style=\"color:var(--text-dim);\">'+m.desc+'</span> <button class=\"btn btn-xs\" onclick=\"downloadModel(\''+dlUrl+'\',\''+m.file+'\',this)\" style=\"float:right;margin-top:-4px;\">📥 下载</button></div>';});html+='</div>';});document.getElementById("recommendedModels").innerHTML=html||'加载中...';}
