...
function renderRecommendedModels(models){var groups={};models.forEach(function(m){if(!groups[m.ram])groups[m.ram]=[];groups[m.ram].push(m);});var html='';Object.keys(groups).forEach(function(ram){html+='<div style=\"margin-bottom:12px;\"><div style=\"font-size:11px;color:var(--accent);margin-bottom:4px;\">💡 '+ram+' 内存推荐</div>';groups[ram].forEach(function(m){html+='<div style=\"padding:8px 10px;margin-bottom:4px;background:var(--bg4);border-radius:6px;font-size:12px;\"><span style=\"color:var(--text-bright);\">'+m.name+'</span> <span style=\"color:var(--text-dim);\">· '+m.brand+' · '+m.size+'</span><br><span style=\"color:var(--text-dim);\">'+m.desc+'</span> <a href=\"'+m.url+'\" target=\"_blank\" style=\"color:var(--accent);float:right;\">📥 下载</a></div>';});html+='</div>';});document.getElementById("recommendedModels").innerHTML=html||'加载中...';}
